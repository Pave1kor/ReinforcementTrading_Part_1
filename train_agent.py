...
def evaluate_model(model, eval_env, deterministic=True):
    ...
    equity_curve = []   # не добавляем initial_equity
    while True:
...
def main():
    ...
    years = [y for y in years if y >= 2018 and y <= 2023]
    windows = []
    ...
    WIN = 60
    NUM_ENVS = 4
    TOTAL_TIMESTEPS = 1_000_000   # увеличено с 500k
    ...
    for win in windows:
        ...
        model = RecurrentPPO(
            ...
            ent_coef=0.03,                     # увеличено
            learning_rate=1e-4,                # увеличено
            n_steps=4096,                      # увеличено
            batch_size=256,                    # увеличено
            n_epochs=4,
            gamma=0.99, gae_lambda=0.95,
            clip_range=0.2, vf_coef=0.3, max_grad_norm=0.7
        )
        ...
        print(f"Inner test {win['name']}: final equity = {test_final:.2f}, Sharpe = {test_sharpe:.3f}, MaxDD = {test_mdd*100:.2f}%")
        all_results.append({'window': win['name'], 'sharpe': test_sharpe, 'mdd': test_mdd, 'final': test_final})
    ...
    final_model = RecurrentPPO(
        ...
        ent_coef=0.03, learning_rate=1e-4, n_steps=4096, batch_size=256, n_epochs=4,
        gamma=0.99, gae_lambda=0.95, clip_range=0.2, vf_coef=0.3, max_grad_norm=0.7
    )
    final_model.learn(total_timesteps=2_000_000)   # увеличено
    ...
    final_sharpe, final_mdd = compute_metrics(final_test_curve, initial_equity=100000.0)

    print(f"\n========== ФИНАЛЬНЫЙ ТЕСТ 2024-2025 ==========")
    print(f"Итоговая эквити: {final_test_equity:.2f} RUB")
    print(f"Sharpe: {final_sharpe:.3f}, MaxDD: {final_mdd*100:.2f}%")
